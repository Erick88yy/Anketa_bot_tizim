...
from middlewares.throttling import ThrottlingMiddleware  # Mahalliy modul
...
# Throttling middleware – 1 soniyada 1 xabar cheklovi
...
async def language_choice(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "1. Ismingizni kiriting:" if user_language == "O'zbek" else 
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "2. Yoshingizni kiriting (16-100):" if language == "O'zbek" else 
...
async def process_age(message: types.Message, state: FSMContext):
    try:
        ...
        if 16 <= age <= 100:
            ...
            language = data.get('language', "O'zbek")
            await message.answer(
                "3. Parametringizni kiriting (Masalan: 178-65-18):" if language == "O'zbek" else 
        ...
        language = data.get('language', "O'zbek")
        await message.answer(
            "Iltimos, yoshingizni to'g'ri kiriting (16-100):" if language == "O'zbek" else 
...
async def process_parameter(message: types.Message, state: FSMContext):
    ...
    if re.match(r'^[\d\-\_\.,\*]+$', parameter):
        ...
        keyboard = ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add(
            KeyboardButton("Aktiv"),
            KeyboardButton("Uni-Aktiv"),
            KeyboardButton("Universal"),
            KeyboardButton("Uni-Passiv"),
            KeyboardButton("Passiv")
        )
        
        await message.answer(
            "4. Temadagi ro'lingizni tanlang:" if language == "O'zbek" else 
            "4. Выберите вашу роль в теме:" if language == "Русский" else 
            "4. Select your role in the topic:",
            reply_markup=keyboard
        )
        await Form.role.set()
    else:
        ...
        language = data.get('language', "O'zbek")
        await message.answer(
            "Iltimos, parametringizni to'g'ri kiriting (Masalan: 178-65-18):" if language == "O'zbek" else 
